...
$(window).scroll(() => {
	...
	//Title Change
	if (scroll <= 170) document.title = "Peter K Joseph >> Welcome";
	else if (scroll <= 850) document.title = "Peter K Joseph >> Overview";
	else document.title = "Peter K Joseph >> Languages";
	trackScale = scroll;
});
...
class DetailedViewer {
	...
	showModal = () => {
		const identifier: string = Math.random().toString(36).substring(7);
